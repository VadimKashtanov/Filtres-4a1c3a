	...
	float * dp)
{
	...
	uint d = blockIdx.z;
	//FOR(0, d, T/BLOQUE) {
	uint cuda_depart_plus_t = cuda_t_MODE_GENERALE(_t_MODE, GRAINE, depart, DEPART, FIN, /*t*/d*BLOQUE_MAX+thx);
		__partage__x[thy][thx] = locd[/*(depart+d*BLOQUE_MAX+thx)*/cuda_depart_plus_t*Y+_y] * dy[/*(depart+d*BLOQUE_MAX+thx)*/cuda_depart_plus_t*Y+_y];//x[(depart+_t)*( X ) + d*BLOQUE + thx];
		__partage__p[thy][thx] = x[/*(depart+(d*BLOQUE_MAX+thy))*/cuda_depart_plus_t*X_vars+DEPART_x +_x];//p[_y*(X+1) + d*BLOQUE + thy];
		__syncthreads();
