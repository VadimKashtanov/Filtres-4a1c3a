	...
	float * locd)
{
	...
	FOR(0, plus_t, _repete_T) {
		...
		uint cuda_depart_plus_t = cuda_t_MODE_GENERALE(_t_MODE, GRAINE, depart, DEPART, FIN, t);
		//
		if (thx < 1) {
			__ret[thy__t][thx] = 0;
		}
		...
		//
		if (thx < 1) {
			locd[/*(0+_t)*/(cuda_depart_plus_t-depart)*BLOQUES*(F_PAR_BLOQUES*1) + BLOQUE*(F_PAR_BLOQUES*1) + _f*1 + thx] = -2*2*__ret[thy__t][thx]*__y[thy__t];
		}
		__syncthreads();
		//
		if (thx < 1) {
			y[/*(0+_t)*/(cuda_depart_plus_t-depart)*BLOQUES*F_PAR_BLOQUES + BLOQUE*F_PAR_BLOQUES + _f] = 2*__y[thy__t] - 1;
		}
	};
};
	...
	float * df)
{
	...
	FOR(0, plus_t, _repete_T) {
		...
		//
		if (thx < 1) {
			__dy0[thy__t] = dy[/*(0+_t)*/(cuda_depart_plus_t-depart)*BLOQUES*F_PAR_BLOQUES + BLOQUE*F_PAR_BLOQUES + _f];
		}
		__syncthreads();
		//
		if (thx < 1) {
			__locd[thy__t][thx] = locd[/*(0+_t)*/(cuda_depart_plus_t-depart)*BLOQUES*(F_PAR_BLOQUES*1) + BLOQUE*(F_PAR_BLOQUES*1) + _f*1 + thx] * __dy0[thy__t]/ (float)(8 - thx);
		}
		__syncthreads();
		//
		xi = x[LIGNE*PRIXS*N_FLTR + /*(depart+_t)*/cuda_depart_plus_t*N_FLTR + thx];
