	...
	float * locd)
{
	...
	if (_t < T && _b < bloques && _f < f_par_bloque) {
		...
		y[/*(0+_t)*/(cuda_depart_plus_t-depart)*(bloques*f_par_bloque) + _b*f_par_bloque + _f] = filtre_device(
			...
			locd  + /*(0+_t)*/(cuda_depart_plus_t-depart)*bloques*f_par_bloque*1 + _b*f_par_bloque*1 + _f*1
		);
	}
};
	...
	float * df)
{
	...
	if (_t < T && _b < bloques && _f < f_par_bloque) {
		...
		d_nvidia_filtre(
			...
			f     + _b*f_par_bloque*N     + _f*N,
			locd  + /*(0+_t)*/(cuda_depart_plus_t-depart)*(bloques*f_par_bloque*1) + _b*(f_par_bloque*1) + _f*1,
			dy    + /*(0+_t)*/(cuda_depart_plus_t-depart)*bloques*f_par_bloque + _b*f_par_bloque + _f,
